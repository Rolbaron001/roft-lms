...
function explain(message: string): string {
  ...
  if (
    ...
    return "Your Claude token is no longer valid - it has been revoked, or it has expired. Nothing is wrong with the platform and nothing was lost. Generate a new one on your own computer with `claude setup-token`, then paste it into Settings. Tokens are yours rather than the tenant's, so this only affects you.";
  }
