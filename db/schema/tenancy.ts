...
export const organisations = pgTable(
  "organisations",
  {
    ...
    /**
     * The other spellings this provider's documents use for a module code.
     *
     * Roland, 21 September: "The codes differ in the documents. Not an
     * importer fault. But it would be good if the system could read
     * permutations of the codes ... This type of fault is going to be
     * continuous and will also affect Tenants who use different codes."
     *
     * One module is KM01 in the curriculum, KM-01 in the alignment table and
     * K1 in a workbook filename, written by different people at different
     * times, and none of the documents is wrong. Punctuation and case were
     * already crossed; a dropped leading zero or a shortened prefix was not,
     * and that failed all fifteen modules of 121151 at once.
     *
     * Canonical code to accepted spellings. Stored here rather than in a table
     * of its own for the same reason as `terminology` and `namingConvention`
     * above: it is a small per-tenant convention read whole, edited whole and
     * never queried across tenants, and a jsonb column inherits this row's
     * isolation instead of needing its own policy.
     *
     * Generated by lib/module-codes.ts and confirmed by a person - see the
     * module codes section in Settings. Ambiguous spellings are withheld with
     * a reason rather than dropped silently, because the first thing somebody
     * does with a missing row is add it back by hand.
     */
    moduleCodeAliases: jsonb("module_code_aliases").$type<
